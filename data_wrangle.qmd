...
I recommend using the keyboard shortcut for pipes because they're annoying to type. Shortcuts can be found in the Help menu.

The pipe passes the object on its left to the first argument of the function to the right of the pipe. In dplyr, the first argument of anything is a data frame, so it's useful to know. It helps me ot think of the pipe as saying "and then". So, in my head I read the code below as "load the package tidyverse, then load the diamonds data frame object and then count the rows in that object."
